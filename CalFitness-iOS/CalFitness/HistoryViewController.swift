...
class HistoryViewController: UIViewController {
    ...
    func updateViewWithRecords() {
        CFStore.getRecordsForLastWeek { (success, objects) in
            ...
            if (objects!.count < 7) {
                for i in 0 ... (6 - (objects!.count)) {
                    records.addObject([
                        Float(0),
                        Float(0),
                        CFDate.beginningOfDate(NSDate().dateByAddingTimeInterval(3600*24*Double(i-6)))
                        ])
                }
            }
            ...
            if !(objects==nil) && (objects?.count>0){
                for i in 0 ... objects!.count-1 {
                    ...
                    records.addObject([
                        object["steps"].floatValue * 1000,
                        object["goal"].floatValue * 1000,
                        CFDate.getDate(object["date"] as? String)
                        ])
                }
            }
